...
def generate_malicious_log(device_id):
    ...
    return {
        "device_id": device_id,
        "failed_connection_ratio": random.uniform(0.4, 0.9),  # High failure rate (e.g., scanning)
        "new_dest_ip_attempts": random.randint(10, 50),     # Connecting to many new IPs
        "port_diversity": random.randint(10, 100),          # Scanning many ports
        "short_session_frequency": random.uniform(0.5, 0.95), # Many short connections
        "suspicious_dns_queries": random.randint(2, 10),    # Unrecognized domains
        "timestamp": datetime.now().isoformat()
    }
    ...
    try:
        while True:
            for device in DEVICES:
                # 80% chance of normal behavior, 20% chance of malicious spike
